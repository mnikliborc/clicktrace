...
public class SystemTestSteps {
	...
	private static void doubleClickSessionAndMaximize(
			FrameFixture editorFixture, int sessionRow) {
		editorFixture.dialog().table()
				.cell(TableCell.row(sessionRow).column(0)).doubleClick();
	...
	private static int findSession(FrameFixture editorFixture, String name) {
		...
		for (; index < rowCount; index++) {
			String value = editorFixture.dialog().table()
					.cell(TableCell.row(index).column(0)).value();
			if (StringUtils.equals(name, value)) {
				return index;
			}
		}
		throw new RuntimeException(String.format("Session '%s' was not found",
				name));
	}
}
